...
// ------Notification-------
// Updated addNotification function for targeted notifications
async function addNotification(notification, specificUser = null) {
  console.log('📨 Sending notification:', notification);
...
// Send notification to admin when user makes request
async function notifyAdminAboutRequest(requestType, userAccount, details = '') {
  const message = `📋 New ${requestType} Request from ${userAccount}${details ? ` - ${details}` : ''}`;
  await addNotification(message, 'admin');
}

// Send notification to user when admin approves/rejects
async function notifyUserAboutRequestStatus(requestType, userAccount, status, details = '') {
  const statusEmoji = status === 'approved' ? '✅' : '❌';
  const message = `${statusEmoji} Your ${requestType} Request ${status}${details ? ` - ${details}` : ''}`;
  await addNotification(message, userAccount);
}

// Updated request functions to notify admin
async function requestAddMoney(){
  const a = +document.getElementById('addAmount').value;
  const m = document.getElementById('addMethod').value;
  const phone = document.getElementById('addPhoneNumber').value.trim();
  ...
  await notifyAdminAboutRequest('Add Money', currentUser, `৳${a} via ${m} - Phone: ${phone}`);
...
async function requestWithdrawMoney(){
  ...
  const n = document.getElementById('withdrawAccount').value.trim();
  ...
  // Notify admin about new request
  await notifyAdminAboutRequest('Withdraw Money', currentUser, `৳${a} via ${m} - Account: ${n}`);
...
async function requestDonation(){
  const a = +document.getElementById('donateAmount').value;
  const n = document.getElementById('donateNote').value;
  ...
  // Notify admin about new request
  await notifyAdminAboutRequest('Donation', currentUser, `৳${a} to BMDSSS0001`);
...
async function requestTransfer(){
  ...
  // Notify admin about new request
  await notifyAdminAboutRequest('Transfer Money', currentUser, `৳${a} to ${to} - Code: ${code}`);
...
async function requestProfileUpdate(){
  ...
  // Notify admin about new request
  await notifyAdminAboutRequest('Profile Update', currentUser);
...
// Updated approveRequest function to notify user
async function approveRequest(requestId) {
  const requests = await getRequests();
  const request = requests.find(r => r.id === requestId);
  if (!request) return;
  ...
  await notifyUserAboutRequestStatus(request.type, request.user, 'approved', 'Processing...');
  ...
  if (request.type === 'New Account') {
    ...
    await notifyUserAboutRequestStatus('New Account', newAccountNumber, 'approved', `Your account number: ${newAccountNumber}`);
  }
  ...
  if (request.type === 'Add') {
    ...
    if (user) {
      ...
      await notifyUserAboutRequestStatus('Add Money', request.user, 'approved', `৳${request.amount} added to your account`);
    }
  }
  ...
  if (request.type === 'Withdraw') {
    ...
    if (user && (user.balance || 0) >= request.amount) {
      ...
      await notifyUserAboutRequestStatus('Withdraw Money', request.user, 'approved', `৳${request.amount} withdrawn from your account`);
    }
  }

  // ... continue for other request types (Donate, Transfer, Forgot Password, Profile Update)
...
// Updated rejectRequest function to notify user
async function rejectRequest(requestId) {
  const requests = await getRequests();
  const request = requests.find(r => r.id === requestId);
  ...
  if (request && request.user) {
    await notifyUserAboutRequestStatus(request.type, request.user, 'rejected');
  }
...
// Updated forgotPassword to notify admin
async function forgotPassword(){
  const accountNo = document.getElementById('login-account').value.trim();
  if(!accountNo) return alert('❌ Enter your account number first!');
  ...
  await notifyAdminAboutRequest('Forgot Password', accountNo);
  
  alert('✅ Password reset request sent to Admin! You will receive a temporary password (123) after approval.');
  updateAdminPanel();
}

// Updated signup to notify admin
async function signup(){
  const n = document.getElementById('signup-name').value.trim();
  const d = document.getElementById('signup-display').value.trim();
  const e = document.getElementById('signup-email').value.trim();
  ...
  await notifyAdminAboutRequest('New Account', 'Pending User', `Name: ${n}, Phone: ${p}`);
